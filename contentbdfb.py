...
def parse_data_content(html,title,icon):
    ...
    if data.get('时效性','') != "失效":
        ...
        if icon and len(tiao_wrap)>0:
            ...
            print(f"运行data_t:---------------------{1}")
        # 判断是否需要加表头
        if not os.path.exists(f'./fabao_result3.csv'):
            data_df.to_csv(f'./fabao_result3.csv', mode='a', index=False)
        else:
            data_df.to_csv(f'./fabao_result3.csv', mode='a', index=False, header=None)
