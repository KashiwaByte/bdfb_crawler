...
def init_driver():
    ...
    # chrome_options.add_argument('user-agent=%s' % "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36")
...
def to_bdfb(driver,entrance=4):
    ...
    #         print('aaa')
    #         continueSend = WebDriverWait(driver, 10, 0.5).until(lambda x: x.find_element(By.XPATH, '//button[@id="proceed-button"]'))
    #         print('bbb')
    #         continueSend.click()
    #         print('已进入发送页面')
    #         time.sleep(2)
...
def to_datalist_page(driver,search_key,del_search_key,keyword="虚假信息"):
    """
    北大法宝页面筛选数据
    ...
    while True:
        try:
            ...
            driver.get(url_login,verify=False)
...
def recognize_code(driver):
    ...
    time.sleep(2)
    if 'drag_bg' in driver.page_source:
        close = WebDriverWait(driver, 20, 0.5).until(lambda x:x.find_element(By.CLASS_NAME,'layui-layer-setwin')).find_element(By.TAG_NAME,'a')
        close.click()
        next_page = WebDriverWait(driver, 20, 0.5).until(lambda x: x.find_element(By.LINK_TEXT, '下一页'))
        next_page.click()
        time.sleep(2)
...
def parse_data_content(html,title,icon):
    ...
    if data.get('时效性','') != "失效":
        ...
        if icon and len(tiao_wrap)>0:
            ...
            print(f"运行data_t:---------------------{1}")
        # 判断是否需要加表头
        if not os.path.exists(f'./difangfatiao.csv'):
            data_df.to_csv(f'./difangfatiao.csv', mode='a', index=False)
        else:
            data_df.to_csv(f'./difangfatiao.csv', mode='a', index=False, header=None)
        ...
        while True:
            while True:
                try:
                    ...
                    time.sleep(3)
            # 获取当前页标题元素列表
            title_list = WebDriverWait(driver, 30, 0.5).until(lambda x:x.find_elements(By.CSS_SELECTOR, '#rightContent > div.grid-right > div:nth-child(1) > div.accompanying-wrap >div.item'))
            ...
            # if next_page:
            next_page = WebDriverWait(driver, 30, 0.5).until(lambda x:x.find_element(By.LINK_TEXT,'下一页'))
            next_page.click()
            time.sleep(5)
            # else:
                # break
